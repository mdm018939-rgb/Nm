...
def strip_country_code(number):
    """নাম্বার থেকে দেশ কোড বাদ দিয়ে বাকি অংশ রিটার্ন করে"""
    for i in [4, 3, 2, 1]:
        code = number[:i]
        if code in COUNTRY_FLAGS:
            return number[i:]
...
number_cc_state = {}   # { (user_id, rid_input): {"full": "8801234567890", "cc_removed": False} }
seen_otp_ids = set()
...
def save_new_number(user_id, rid_input, number):
    ...
    with otp_lock:
        ...
        number_cc_state[(user_id, rid_input)] = {"full": number, "cc_removed": False}
...
def replace_number_for_range(user_id, rid_input, number):
    ...
    with otp_lock:
        ...
        prev_cc_removed = number_cc_state.get(key, {}).get("cc_removed", False)
        number_cc_state[key] = {"full": number, "cc_removed": prev_cc_removed}
...
def _fetch_and_send_number(chat_id, user_id, rid_input, loading_message_id):
    ...
    if data and data.get("meta", {}).get("code") == 200:
        ...
        markup.row(
            types.InlineKeyboardButton("🚫 Delete Country Code", callback_data=f"cc_{rid_input}", style="secondary")
        )
        markup.row(
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("cc_"))
def handle_toggle_cc(call):
    user_id = call.from_user.id
    rid_input = call.data.split("_", 1)[1]
    key = (user_id, rid_input)
    key2 = (user_id, rid_input + "_2")

    state = number_cc_state.get(key)
    if not state:
        bot.answer_callback_query(call.id, text="❌ Number not found.", show_alert=True)
        return

    state["cc_removed"] = not state["cc_removed"]
    new_flag = state["cc_removed"]
    full_num = state["full"]
    display_num = strip_country_code(full_num) if new_flag else full_num
    cc_label = "➕ Add Country Code" if new_flag else "🚫 Delete Country Code"

    state2 = number_cc_state.get(key2)
    full_num2 = None
    display_num2 = None
    if state2:
        state2["cc_removed"] = new_flag
        full_num2 = state2["full"]
        display_num2 = strip_country_code(full_num2) if new_flag else full_num2
    ...
    for row in markup.keyboard:
        for btn in row:
            if btn.callback_data == call.data:
                btn.text = cc_label
            elif getattr(btn, "copy_text", None) and btn.copy_text.text in (full_num, strip_country_code(full_num)):
                btn.text = display_num
                btn.copy_text = types.CopyTextButton(text=display_num)
            elif full_num2 and getattr(btn, "copy_text", None) and btn.copy_text.text in (full_num2, strip_country_code(full_num2)):
                btn.text = display_num2
                btn.copy_text = types.CopyTextButton(text=display_num2)
...
def _fetch_and_send_changed_number(user_id, rid_input, chat_id, message_id):
    ...
    if data and data.get("meta", {}).get("code") == 200:
        ...
        key = (user_id, rid_input)
        cc_state = number_cc_state.get(key, {"cc_removed": False})
        cc_label = "➕ Add Country Code" if cc_state["cc_removed"] else "🚫 Delete Country Code"

        markup = types.InlineKeyboardMarkup()
        full_num = num_data.get('full_number') or num_data.get('no_plus_number', 'N/A')
        display_num = strip_country_code(full_num) if cc_state["cc_removed"] else full_num
        markup.add(types.InlineKeyboardButton(
            text=display_num,
            copy_text=types.CopyTextButton(text=display_num),
            style="success"
        ))
        if num_data2:
            full_num2 = num_data2.get('full_number') or num_data2.get('no_plus_number', 'N/A')
            display_num2 = strip_country_code(full_num2) if cc_state["cc_removed"] else full_num2
            key2 = (user_id, rid_input + "_2")
            if key2 in number_cc_state:
                number_cc_state[key2]["cc_removed"] = cc_state["cc_removed"]
            markup.add(types.InlineKeyboardButton(
                text=display_num2,
                copy_text=types.CopyTextButton(text=display_num2),
                style="success"
            ))
        markup.row(
            types.InlineKeyboardButton(cc_label, callback_data=f"cc_{rid_input}", style="secondary")
        )
        markup.row(
            types.InlineKeyboardButton("🔄 Change Number", callback_data=f"change_{rid_input}", style="primary")
        )
