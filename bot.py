...
def process_range(message):
    ...
    if message.chat.type != "private":
        return
    if not message.text:
        bot.send_message(message.chat.id, "❌ Please send the range ID as text (e.g., 123456XXX).")
        return
    ...
    loading = bot.send_message(message.chat.id, "🔍 Searching for a number, please wait...", reply_markup=main_keyboard())
...
def _fetch_and_send_number(chat_id, user_id, rid_input, loading_message_id):
    ...
    if data and data.get("meta", {}).get("code") == 200:
        ...
        bot.delete_message(chat_id, loading_message_id)
        bot.send_message(chat_id, "❌ No numbers available in this range. Try a different range.")
