...
def _fetch_and_send_number(chat_id, user_id, rid_input, loading_message_id):
    ...
    if data and data.get("meta", {}).get("code") == 200:
        ...
        markup.row(
            types.InlineKeyboardButton("🗑 Delete Country Code", callback_data=f"delcc_{rid_input}", style="danger")
        )
        markup.row(
            types.InlineKeyboardButton("🔄 Change Number", callback_data=f"change_{rid_input}_0", style="primary")
        )
        markup.row(
...
# 🗑 Delete/Add Country Code হ্যান্ডলার
@bot.callback_query_handler(func=lambda call: call.data.startswith("delcc_") or call.data.startswith("addcc_"))
def handle_country_code(call):
    ...
    with otp_lock:
        num1 = user_range_number.get((user_id, rid_input), "")
        num2 = user_range_number.get((user_id, rid_input + "_2"), "")

    def strip_cc(number):
        for i in [4, 3, 2, 1]:
            if number[:i] in COUNTRY_FLAGS:
                return number[i:]
        return number

    if action == "delcc":
        show_num1 = strip_cc(num1) if num1 else "N/A"
        show_num2 = strip_cc(num2) if num2 else None
        btn_label = "➕ Add Country Code"
        btn_prefix = "addcc"
    else:
        show_num1 = "+" + num1 if num1 else "N/A"
        show_num2 = "+" + num2 if num2 else None
        btn_label = "🗑 Delete Country Code"
        btn_prefix = "delcc"

    flag_num = num1.lstrip('+')
    result_text = (
        f"✅ **Number Assigned Successfully!**\n\n"
        f"🌐 **Country:** {get_country_flag(flag_num)[0]} ({get_country_flag(flag_num)[1]})\n"
        f"🎯 **Range:** `{rid_input}`\n\n"
        f"🌀 **OTP Forwarded Automatically.**"
    )
    ...
    markup.add(types.InlineKeyboardButton(
        text=show_num1,
        copy_text=types.CopyTextButton(text=show_num1),
        style="success"
    ))
    if show_num2:
        markup.add(types.InlineKeyboardButton(
            text=show_num2,
            copy_text=types.CopyTextButton(text=show_num2),
            style="success"
        ))
    markup.row(
        types.InlineKeyboardButton(btn_label, callback_data=f"{btn_prefix}_{rid_input}", style="danger" if btn_prefix == "delcc" else "primary")
    )
    markup.row(
        types.InlineKeyboardButton("🔄 Change Number", callback_data=f"change_{rid_input}_{'1' if action == 'delcc' else '0'}", style="primary")
    )
    markup.row(
        types.InlineKeyboardButton("🔔 OTP GROUP", url=OTP_GROUP_LINK, style="danger")
    )
    ...
    try:
        bot.edit_message_text(result_text, chat_id=call.message.chat.id, message_id=call.message.message_id, parse_mode="Markdown", reply_markup=markup)
    except Exception as e:
...
def handle_change_number(call):
    ...
    parts = call.data.split("_")
    rid_input = parts[1]
    cc_removed = parts[2] == "1" if len(parts) > 2 else False
    ...
    threading.Thread(
        ...
        args=(call.from_user.id, rid_input, call.message.chat.id, call.message.message_id, cc_removed),
        daemon=True
    ).start()

def _fetch_and_send_changed_number(user_id, rid_input, chat_id, message_id, cc_removed=False):
    try:
        data, data2 = request_two_numbers_parallel(rid_input)
    except Exception as e:
    ...
    if data and data.get("meta", {}).get("code") == 200:
        ...
        markup = types.InlineKeyboardMarkup()
        full_num = num_data.get('full_number') or num_data.get('no_plus_number', 'N/A')
        show_num = re.sub(r'^\+\d{1,4}', '', full_num) if cc_removed else full_num
        markup.add(types.InlineKeyboardButton(
            text=show_num,
            copy_text=types.CopyTextButton(text=show_num),
            style="success"
        ))
        if num_data2:
            full_num2 = num_data2.get('full_number') or num_data2.get('no_plus_number', 'N/A')
            show_num2 = re.sub(r'^\+\d{1,4}', '', full_num2) if cc_removed else full_num2
            markup.add(types.InlineKeyboardButton(
                text=show_num2,
                copy_text=types.CopyTextButton(text=show_num2),
                style="success"
        ...
        markup.row(
            types.InlineKeyboardButton("➕ Add Country Code" if cc_removed else "🗑 Delete Country Code", callback_data=f"{'addcc' if cc_removed else 'delcc'}_{rid_input}", style="primary" if cc_removed else "danger")
        )
        markup.row(
