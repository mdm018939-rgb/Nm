...
# ──────────────────────────────────────────────────────────
# {(chat_id, message_id): {"rid": rid_input, "num1": no_plus_number, "num2": no_plus_number বা None, "cc_on": True/False}}
...
def build_result_markup(rid_input, num1, num2, cc_on):
    ...
    if cc_on:
        cc_button = types.InlineKeyboardButton("➖ Remove CC", callback_data=f"togglecc_{rid_input}", style="danger")
    else:
...
def handle_toggle_cc(call):
    ...
    try:
        ...
        bot.answer_callback_query(call.id, text="✅ CC Removed" if not state["cc_on"] else "✅ CC Added")
    except Exception as e:
