...
def strip_country_code(no_plus_number):
    """নাম্বারের শুরু থেকে Country Code অংশটা বাদ দিয়ে বাকি অংশ রিটার্ন করে"""
    for i in [4,3,2,1]:
        code = no_plus_number[:i]
        if code in COUNTRY_FLAGS:
            return no_plus_number[i:]
    return no_plus_number

# ──────────────────────────────────────────────────────────
# 🔘 Remove CC / Add CC টগল বাটন — নতুন যোগ করা অংশ
# ──────────────────────────────────────────────────────────
message_number_state = {}

def format_number_display(no_plus_number, cc_on):
    """cc_on=True হলে +CC সহ নাম্বার, False হলে CC বাদ দিয়ে নাম্বার রিটার্ন করে"""
    if cc_on:
        return f"+{no_plus_number}"
    return strip_country_code(no_plus_number)

def build_result_markup(rid_input, num1, num2, cc_on):
    """নাম্বার(গুলো) + Change Number + Remove/Add CC + OTP Group বাটন সহ মার্কআপ বানায়"""
    markup = types.InlineKeyboardMarkup()

    disp1 = format_number_display(num1, cc_on)
    markup.add(types.InlineKeyboardButton(
        text=disp1,
        copy_text=types.CopyTextButton(text=disp1),
        style="success"
    ))
    if num2:
        disp2 = format_number_display(num2, cc_on)
        markup.add(types.InlineKeyboardButton(
            text=disp2,
            copy_text=types.CopyTextButton(text=disp2),
            style="success"
        ))
    ...
    if cc_on:
        cc_button = types.InlineKeyboardButton("➖ Remove CC", callback_data=f"togglecc_{rid_input}", style="success")
    else:
        cc_button = types.InlineKeyboardButton("➕ Add CC", callback_data=f"togglecc_{rid_input}", style="success")
    markup.row(cc_button)
    markup.row(
...
def replace_number_for_range(user_id, rid_input, number):
    """🔄 Change Number থেকে কল হয়: পুরনো নাম্বারটা active_numbers থেকে সরানো হয় না —
    সেটা তার নিজের ২০ মিনিট মেয়াদ শেষ না হওয়া পর্যন্ত সক্রিয় থাকবে এবং OTP পেতে থাকবে।
    শুধু user_range_number আপডেট হয়, যাতে এই রেঞ্জের জন্য পরবর্তী Change Number নতুন নাম্বার থেকে কাজ করে।"""
    with otp_lock:
...
def poll_otps():
    ...
    while True:
        ...
        # 🧹 message_number_state ক্লিনআপ — যেসব মেসেজের নাম্বার(গুলো) আর কোনোটাই
        # active_numbers এ নেই (সব এক্সপায়ার হয়ে গেছে), সেগুলোর CC-state এন্ট্রিও মুছে ফেলা হচ্ছে
        with otp_lock:
            dead_keys = []
            for msg_key, state in message_number_state.items():
                num1_alive = state.get("num1") in active_numbers
                num2_alive = state.get("num2") in active_numbers if state.get("num2") else False
                if not num1_alive and not num2_alive:
                    dead_keys.append(msg_key)
            for msg_key in dead_keys:
                del message_number_state[msg_key]
...
def request_number(rid_input, max_retries=10, retry_delay=0.5):
    headers = {
        "mauthapi": API_KEY, 
        "Content-Type": "application/json",
        "User-Agent": "Mozilla/5.0"
...
def _fetch_and_send_number(chat_id, user_id, rid_input, loading_message_id):
    ...
    if data and data.get("meta", {}).get("code") == 200:
        ...
        num1_no_plus = num_data.get('no_plus_number', 'N/A')
        num2_no_plus = num_data2.get('no_plus_number') if num_data2 else None
        markup = build_result_markup(rid_input, num1_no_plus, num2_no_plus, cc_on=True)

        bot.delete_message(chat_id, loading_message_id)
        sent = bot.send_message(chat_id, result_text, parse_mode="Markdown", reply_markup=markup)

        # 🔘 নতুন GET NUMBER সবসময় ফ্রেশ state (CC অন) দিয়ে শুরু হবে
        message_number_state[(sent.chat.id, sent.message_id)] = {
            "rid": rid_input,
            "num1": num1_no_plus,
            "num2": num2_no_plus,
            "cc_on": True
...
def handle_change_number(call):
    ...
    bot.edit_message_text("⏳ Requesting change number...", chat_id=call.message.chat.id, message_id=call.message.message_id)
    ...
    threading.Thread(
        ...
        args=(call.from_user.id, rid_input, call.message.chat.id, call.message.message_id),
        daemon=True
    ).start()
...
def _fetch_and_send_changed_number(user_id, rid_input, chat_id, message_id):
    ...
    if data and data.get("meta", {}).get("code") == 200:
        ...
        # 🔘 আগে এই মেসেজে CC অন ছিল না অফ ছিল, সেই state ধরে রাখা হচ্ছে (না থাকলে ডিফল্ট True)
        key = (chat_id, message_id)
        prev_cc_on = message_number_state.get(key, {}).get("cc_on", True)

        markup = build_result_markup(rid_input, num1_no_plus, num2_no_plus, cc_on=prev_cc_on)
        message_number_state[key] = {
            "rid": rid_input,
            "num1": num1_no_plus,
            "num2": num2_no_plus,
            "cc_on": prev_cc_on
...
# 🔘 Remove CC / Add CC টগল বাটন ক্লিকের হ্যান্ডলার
@bot.callback_query_handler(func=lambda call: call.data.startswith("togglecc_"))
def handle_toggle_cc(call):
    chat_id = call.message.chat.id
    message_id = call.message.message_id
    key = (chat_id, message_id)

    state = message_number_state.get(key)
    if not state:
        bot.answer_callback_query(call.id, text="⚠️ Session expired. Please get a new number.", show_alert=True)
        return

    # টগল করা হচ্ছে: CC অন থাকলে অফ, অফ থাকলে অন
    state["cc_on"] = not state["cc_on"]

    markup = build_result_markup(state["rid"], state["num1"], state.get("num2"), state["cc_on"])

    try:
        bot.edit_message_reply_markup(chat_id=chat_id, message_id=message_id, reply_markup=markup)
        bot.answer_callback_query(call.id)
    except Exception as e:
        print(f"❌ handle_toggle_cc এরর: {e}")
        bot.answer_callback_query(call.id, text="❌ Something went wrong. Try again.", show_alert=True)
