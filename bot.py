...
# ❌ Cancel বাটন হ্যান্ডলার — GET NUMBER-এর রেঞ্জ-চাওয়া ধাপ বাতিল করে ও মেনু ফিরিয়ে দেয়
@bot.callback_query_handler(func=lambda call: call.data == "cancel_get_number")
def handle_cancel_get_number(call):
    waiting_users.discard(call.from_user.id)
    bot.clear_step_handler_by_chat_id(call.message.chat.id)
    bot.answer_callback_query(call.id, text="❌ Canceled")
    try:
        bot.edit_message_text("❌ Canceled", chat_id=call.message.chat.id, message_id=call.message.message_id)
    except:
        pass
    send_welcome_menu(call.message.chat.id)
...
def handle_text(message):
    ...
    if message.text == "📱 GET NUMBER":
        ...
        cancel_markup.add(types.InlineKeyboardButton("❌ Cancel", callback_data="cancel_get_number"))
        msg = bot.send_message(
            message.chat.id,
            "⌨️ **Enter Range ID (e.g., 123456XXX):**",
            parse_mode="Markdown",
