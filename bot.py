...
def main_keyboard():
    ...
    markup.add(types.KeyboardButton("📱 GET NUMBER"), types.KeyboardButton("🔍 View Range"))
...
def send_welcome(message):
    ...
    if len(parts) > 1:
        ...
        if is_user_joined(message.from_user.id):
            bot.send_message(message.chat.id, "📱 GET NUMBER")
            fake_msg = message
            fake_msg.text = rid_input
            process_range(fake_msg)
        else:
...
def handle_text(message):
    ...
    if message.text == "📱 GET NUMBER":
        ...
        help_text = (
            ...
            "1️⃣ প্রথমে বটে /start কমান্ড দিন, তাহলে মেইন মেনু দেখতে পাবেন\n\n"
            "2️⃣ কোন কোন রেঞ্জে এখন নাম্বার আছে সেটা জানতে 🔍 <b>View Range</b> বাটনে ক্লিক করুন অথবা Live Access লিংকে যান\n\n"
            "3️⃣ সেখানে রেঞ্জ এভাবে দেখাবে: <code>1234567XXX</code> — এই সম্পূর্ণ রেঞ্জটা কপি 📋 করে নিন\n\n"
            "4️⃣ বটে ফিরে এসে 📱 <b>GET NUMBER</b> বাটনে ক্লিক করুন, তারপর যে রেঞ্জটা কপি করেছিলেন সেটা পাঠান (রিপ্লাই আকারে)\n\n"
            "5️⃣ ✅ সেই মুহূর্তে যদি ওই রেঞ্জে কোনো নাম্বার খালি থাকে, সাথে সাথে আপনাকে একটা নাম্বার দেওয়া হবে\n"
            "❌ যদি খালি না থাকে, তাহলে একটু পর আবার একই রেঞ্জে চেষ্টা করুন, বা অন্য কোনো রেঞ্জ ব্যবহার করুন\n\n"
            "6️⃣ নাম্বার পেয়ে গেলে সেটা কপি করে যেখানে ভেরিফিকেশনের জন্য দরকার সেখানে ব্যবহার করুন\n\n"
            "7️⃣ 🔑 ওই নাম্বারে কোনো OTP/Code আসলে সেটা বট নিজে থেকেই আপনার ইনবক্সে (DM-এ) পাঠিয়ে দেবে। যদি কিছুক্ষণেও না আসে, তাহলে OTP গ্রুপে গিয়ে চেক করুন\n\n"
            "⚠️ কোনো ধাপে আটকে গেলে বা সমস্যা হলে নির্দ্বিধায় 💬 সাপোর্টে যোগাযোগ করুন — আমরা সাহায্য করতে প্রস্তুত"
        )
...
def process_range(message):
    ...
    if message.text == "/start":
        ...
        return
    if message.text in ("📱 GET NUMBER", "🔍 View Range","❓ Help","💬 Support"):
        handle_text(message)
        return
    ...
    data2 = request_number(rid_input)
    ...
    if data and data.get("meta", {}).get("code") == 200:
        ...
        if num_data2:
            save_new_number(message.from_user.id, rid_input + "_2", num_data2.get("no_plus_number", ""))
        result_text = (
            f"✅ **Number Assigned Successfully!**\n\n"
            f"🎯 **Range:** `{rid_input}`\n"
            f"🌐 **Country:** {get_country_flag(num_data.get('no_plus_number', ''))} {num_data.get('country')}\n\n"
            f"🌀 **OTP Forwarded Automatically."
        )
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton(
            text=num_data.get('full_number'),
            copy_text=types.CopyTextButton(text=num_data.get('full_number'))
        ))
        if num_data2:
            markup.add(types.InlineKeyboardButton(
                text=num_data2.get('full_number'),
                copy_text=types.CopyTextButton(text=num_data2.get('full_number'))
            ))
        markup.row(
            types.InlineKeyboardButton("🔄 Change Number", callback_data=f"change_{rid_input}")
        )
        markup.row(
...
def handle_change_number(call):
    ...
    if data and data.get("meta", {}).get("code") == 200:
        ...
        if num_data2:
            replace_number_for_range(call.from_user.id, rid_input + "_2", num_data2.get("no_plus_number", ""))
        updated_text = (
